...
#pragma config FPLLMUL      = MUL_15   // Multiply PLL by 7 (=60MHz core clock)
#pragma config FPLLODIV     = DIV_2     // PLL output divide by 1 (=30MHZ Fcy)
#pragma config FPBDIV       = DIV_2     // Divide core clock by 2 for peripheral bus (=15MHz PB clock)
...
void __ISR (_TIMER_1_VECTOR, ipl2) TMR1IntHandler(void) {
    ...
    for (y = Y_MAX; y >= 0; y--) {
        for (x = X_MAX; x >= 0; x--) {
            // Odd-numbered Y-offsets are wired backwards (due to my own impatience). Need to reverse the shift order on the X-axis in this case.
            // Remove the (7-x) X-offset hack if/when the wiring is corrected.
            if ((y % 2) != 0) {
                if (cube_fb[(7-x)][y][z].r >> bcm_bit_pos)
                    LATBSET = SR_SDI_R;

                if (cube_fb[(7-x)][y][z].g >> bcm_bit_pos)
                    LATBSET = SR_SDI_G;

                if (cube_fb[(7-x)][y][z].b >> bcm_bit_pos)
                    LATBSET = SR_SDI_B;
            }
            // If we're on an even-numbered Y-offset, shift the data normally.
            else {
                if (cube_fb[x][y][z].r >> bcm_bit_pos)
                    LATBSET = SR_SDI_R;

                if (cube_fb[x][y][z].g >> bcm_bit_pos)
                    LATBSET = SR_SDI_G;

                if (cube_fb[x][y][z].b >> bcm_bit_pos)
                    LATBSET = SR_SDI_B;
            }
      ...
      // Turn on the current layer and blinken ze lights
       ...
       * the layer transistor previously ON appears to still conduct slightly
      ...
      switch (cube_layer) {
        case (0) :
            LATESET = LAYER_Q7;
            for (delay = 0; delay < Q_DELAY; delay++) {}
            LATECLR = LAYER_Q0;
             break;
        case (1) :
            LATESET = LAYER_Q0;
...
int main() {
    ...
    /*** WORKING OK, MIGHT NEED TO TUNE TIMING FOR BRIGHTNESS, ETC. ****/
    OpenTimer1( T1_ON | T1_SOURCE_INT | T1_PS_1_8, 600);    // Enable TMR1, use internal clock source, 1:8 PS, PR = 150
    ConfigIntTimer1( T1_INT_ON | T1_INT_PRIOR_2);   // Enable T1 Interrupt with Priority 2
