...
/* WIP
 * 
 * NONE OF THESE FUNCTIONS HAVE BEEN TESTED YET
 */
...
const unsigned char bitmaps[8][8] = {
...
const unsigned char paths[44] = {
    ...
    0x47,0x37,0x26,0x15 }; // circle, len 16, offset 28
...
void Font_Getchar_Flipped (char chr, unsigned char *dst) {
	uint8_t i;
//	unsigned char *tmp = 0;
	chr -= 32; // our bitmap font starts at ascii char 32.

	// loop through each byte of the array and store slice in tmp[i] until entire char is built
	// the char will be backwards however so we need to flip each byte in the array before storing in *dst
    for (i = 0; i < 5; i++) {
		dst[(i)] = font[(chr*5) + (5-i) ];
	}
}

void Font_Getbitmap (char bitmap, unsigned char *dst) {
	int i;
